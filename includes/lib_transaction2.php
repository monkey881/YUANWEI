    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	    ...
	    else if($row['status']==3)
	   {
        ...
        $arr[] = array('tid'       => $row['tid'],
					   ...
					   'y_weight'   => ($row['taocan_weight']-$row['song_weight'])*2 .'斤',
	...
	if($res)
	{
	    ...
	    else if($res['status']==4)
	   {
		   $res['handler'] = " <a href=\"user.php?act=stop_taocan&tid=" .$res['tid']. "\" onclick=\"if (!confirm('你确认要开始配送吗')) return false;\">开始配送</a>";
					   ...
					   $res['y_weight']   = ($res['taocan_weight']-$res['song_weight']) .'斤';
	}
...
{
    $sql = "SELECT ps_time FROM " .$GLOBALS['ecs']->table('peisong') . " WHERE order_taocan_id = '$tid' and status='$status'";
